...
| `KUBE_CLI` | `kubectl` | kubectl binary to use (set to `oc` automatically on OpenShift when `oc` is detected) |
...
| `--postgres-ocp-image IMAGE` | PostgreSQL (OpenShift — CNPG operand image) |
...
| `POSTGRES_OCP_IMAGE` | PostgreSQL (OpenShift) |
...
# OpenShift with custom Causa Backend image and target app URL
export CAUSA_MCP_QUARKUS_METRICS_BASE_URL="http://my-quarkus-app.default.svc.cluster.local:8080"
./install.sh --target openshift --causa-backend-image quay.io/myorg/causa-backend:v1.2.3
```
